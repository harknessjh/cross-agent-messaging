...
- A successful generic file read or envelope validation does not prove those directory controls. The reference reader refuses a final-component symlink, opens path inputs nonblocking before accepting only regular files, and bounds the bytes read; it does not authenticate an arbitrary path's ancestor chain. Select and verify the private exchange directory through the trusted operator workflow.
