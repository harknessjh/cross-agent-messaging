...
CONVERSATION_JOURNAL_EVENT_TYPES = frozenset(
    {"message.outbound.intent", "message.inbound.observed", "message.inbound.validated"}
)
_SCHEMA_PATH = (
...
def _messages(records: Sequence[Mapping[str, Any]]) -> dict[str, _Message]:
...
def build_outbound_link(
    ...
    *,
    ...
    if retry_after_intent is not None:
        ...
        return parse_link(matches[0]["attributes"].get("conversation_link"))
