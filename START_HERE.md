...
Each agent always pauses for two project-specific confirmations during first-time enrollment. It may also pause for one account-level executable approval if that exact product fingerprint has not already been approved:
...
Separately, CAM records account-wide product executable approvals in the owner-private append-only ledger `~/CAM/Approvals/product-executables-v1.jsonl`. That ledger is not a project journal and does not add files to an application worktree.
