...
class ProductApprovalRecoveryTests(ProductApprovalRecoveryTestCase):
    ...
    def test_retry_resyncs_manifest_directory_before_truncate(self) -> None:
        ...
        def fail_manifest_directory_sync(descriptor: int) -> None:
            nonlocal directory_sync_count
            if stat.S_ISDIR(os.fstat(descriptor).st_mode):
                directory_sync_count += 1
                if directory_sync_count == 2:
                    raise OSError(
                        errno.EIO, "synthetic manifest directory fsync failure"
                    )
            real_fsync(descriptor)

        with (
            mock.patch.object(evidence.os, "fsync", fail_manifest_directory_sync),
            self.assertRaises(
                product_approval_recovery.RecoveryMutationError
            ) as failed,
        ):
        ...
        self.assertEqual(failed.exception.audit["mutation_state"], "not_attempted")
        self.assertEqual(self.registry.read_bytes(), damaged)
        self.assertEqual(directory_sync_count, 2)
        ...
        def truncate_after_sync(descriptor: int, length: int) -> None:
            self.assertTrue(directory_synced)
            real_ftruncate(descriptor, length)

        with (
            mock.patch.object(evidence.os, "fsync", observe_sync),
            mock.patch.object(
                product_approval_recovery.os,
                "ftruncate",
                side_effect=truncate_after_sync,
            ),
        ):
            result = product_approvals.recover_partial_tail(**arguments)

        self.assertTrue(directory_synced)
        self.assertEqual(result["status"], "recovered_partial_tail")
        self.assertEqual(self.registry.read_bytes(), prefix)
